...
it('creates a keyed dictionary of routes for multiple routes pointing to the same action', () => {
    expect(same['/two-routes-one-action-1'].url()).toBe('/two-routes-one-action-1')
    expect(same['/two-routes-one-action-1']()).toEqual({
        uri: '/two-routes-one-action-1',
        method: 'get',
    })

    expect(same['/two-routes-one-action-2'].url()).toBe('/two-routes-one-action-2')
    expect(same['/two-routes-one-action-2']()).toEqual({
        uri: '/two-routes-one-action-2',
        method: 'get',
    })
})
